...
parser.add_argument('--rgb_root', type=str, default='./dataset/train/S0/',
                    help='the training rgb images root')  # train_dut
parser.add_argument('--depth_root', type=str, default='./dataset/train/DOP_Gray/',
                    help='the training depth images root')
parser.add_argument('--gt_root', type=str, default='./dataset/train/gtmask/',
                    help='the training gt images root')

parser.add_argument('--val_rgb', type=str, default='./dataset/test/S0/',
                    help='validate rgb path')
parser.add_argument('--val_depth', type=str, default='./dataset/test/DOP_Gray/',
                    help='validate depth path')
parser.add_argument('--val_gt', type=str, default='./dataset/test/gtmask/',
...
parser.add_argument('--save_path', type=str, default="ckps/PBDNet/", help='checkpoint path')
...
def validate(test_dataset, model, epoch, opt):
    ...
    if epoch == 0:
        ...
        if mae < best_mae:
            ...
            best_epoch = epoch
            torch.save(model.state_dict(), opt.save_path + 'PBDNet_mae_best.pth', _use_new_zipfile_serialization=False)
